...
def read_data():
    # features_matrix, training_labels = read_csv()
    # numpy.save('census_features.npy', features_matrix)
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(features_matrix, training_labels, test_size=0.5)
...
def read_csv():
    ...
    dataframe = replace_columns(dataframe)
    print("Finished processing columns data")
    features_matrix = dataframe.as_matrix(list(range(0, 41)))
    training_labels = dataframe[41].tolist()
    return features_matrix, training_labels


def replace_columns(dataframe):
    col1vals = {' Without pay': 1, ' Local government': 2, ' Private': 3, ' State government': 4,
                ' Federal government': 5,
                ' Not in universe': 0, ' Self-employed-incorporated': 6, ' Never worked': 7,
                ' Self-employed-not incorporated': 8}
    col4vals = {' Doctorate degree(PhD EdD)': 0, ' Masters degree(MA MS MEng MEd MSW MBA)': 1,
                ' 1st 2nd 3rd or 4th grade': 2,
                ' Associates degree-academic program': 3, ' 11th grade': 4, ' 10th grade': 5, ' 9th grade': 6,
                ' Children': 7,
                ' Associates degree-occup /vocational': 8, ' 5th or 6th grade': 9, ' 7th and 8th grade': 10,
                ' Some college but no degree': 11, ' High school graduate': 12,
    ...
    col8vals = {' Business and repair services': 1, ' Communications': 2, ' Construction': 3,
                ' Manufacturing-durable goods': 4,
                ' Education': 5, ' Forestry and fisheries': 7, ' Mining': 8, ' Not in universe or children': 9,
                ' Social services': 10, ' Entertainment': 11, ' Manufacturing-nondurable goods': 12,
                ' Transportation': 13,
                ' Agriculture': 14, ' Armed Forces': 22, ' Hospital services': 16, ' Medical except hospital': 17,
                ' Wholesale trade': 18, ' Retail trade': 19, ' Public administration': 20,
                ' Other professional services': 21,
                ' Personal services except private HH': 22, ' Utilities and sanitary services': 23,
                ' Private household services': 24,
                ' Farming forestry and fishing': 25, ' Handlers equip cleaners etc ': 26,
                ' Technicians and related support': 27,
                ' Machine operators assmblrs & inspctrs': 28, ' Executive admin and managerial': 29,
                ' Professional specialty': 30,
                ' Precision production craft & repair': 31, ' Adm support including clerical': 32, ' Sales': 33,
                ' Other service': 34, ' Protective services': 35, ' Transportation and material moving': 36,
                ' Finance insurance and real estate': 37}
    col10vals = {' Other': 0, ' Amer Indian Aleut or Eskimo': 1, ' Black': 2, ' Asian or Pacific Islander': 3,
                 ' White': 4}
    col11vals = {' Cuban': 0, ' Mexican-American': 1, ' Central or South American': 2, ' Other Spanish': 3,
                 ' Chicano': 4,
                 ' All other': 5, ' Puerto Rican': 6, ' NA': 7, ' Do not know': 8, ' Mexican (Mexicano)': 9}
    col12vals = {' Male': 0, ' Female': 1}
    col14vals = {' New entrant': 1, ' Job leaver': 2, ' Job loser - on layoff': 3, ' Re-entrant': 4,
                 ' Other job loser': 5}
    ...
    col20vals = {' Abroad': 1, ' West': 2, ' Midwest': 3, ' South': 4, ' Northeast': 5, ' Missouri': 6,
                 ' New Jersey': 7, ' Texas': 8, ' Mississippi': 9, ' Arkansas': 10, ' District of Columbia': 11,
                 ' Florida': 12, ' Kansas': 13, ' California': 14, ' Idaho': 15, ' Ohio': 16, ' Maine': 17, ' Oklahoma': 18,
                 ' Alabama': 19, ' Louisiana': 20, ' New Hampshire': 21, ' Wisconsin': 22, ' Georgia': 23, ' Iowa': 24,
                 ' West Virginia': 25, ' Delaware': 26, ' Vermont': 27, ' Michigan': 29, ' Nebraska': 30, ' New York': 31,
                 ' Illinois': 32, ' Arizona': 33, ' Wyoming': 34, ' North Carolina': 35, ' Indiana': 36,
                 ' Minnesota': 37, ' Alaska': 38, ' Montana': 39, ' Virginia': 40, ' Utah': 41, ' Connecticut': 42, ' South Dakota': 43,
                 ' Pennsylvania': 44, ' Tennessee': 45, ' New Mexico': 46, ' Massachusetts': 47, ' Maryland': 48,
                 ' Nevada': 49, ' North Dakota': 50, ' Colorado': 51, ' Oregon': 52, ' South Carolina': 53, ' Kentucky': 54}
    col22vals = {' Grandchild <18 never marr child of subfamily RP': 0, ' Child 18+ spouse of subfamily RP': 1,
                 ...
                 ' Child <18 spouse of subfamily RP': 8, ' Secondary individual': 9,
                 ' Other Rel 18+ ever marr RP of subfamily': 10,
                 ' Child <18 ever marr not in subfamily': 11, ' Child <18 never marr RP of subfamily': 12,
                 ' Grandchild <18 ever marr not in subfamily': 13, ' Other Rel <18 spouse of subfamily RP': 14,
                 ' Nonfamily householder': 15,
                 ' Other Rel 18+ ever marr not in subfamily': 16, ' Child <18 ever marr RP of subfamily': 17,
                 ' In group quarters': 18,
                 ...
                 ' Child under 18 ever married': 40, ' Child under 18 never married': 41,
                 ' Nonrelative of householder': 42,
                 ' Child 18 or older': 39}
    col25vals = {' MSA to MSA': 0, ' NonMSA to MSA': 1, ' Not identifiable': 2, ' MSA to nonMSA': 3,
                 ' Abroad to MSA': 4,
                 ' Abroad to nonMSA': 5, ' Nonmover': 6, ' NonMSA to nonMSA': 7}
    col26vals = {' Same county': 2, ' Different region': 3, ' Different division same region': 4,
                 ' Different state same division': 8,
    ...
    col31vals = {' Mother only present': 1, ' Father only present': 2, ' Neither parent present': 3,
                 ' Both parents present': 4}
    col32vals = {' El-Salvador': 23, ' Outlying-U S (Guam USVI etc)': 1, ' Portugal': 2, ' Trinadad&Tobago': 3,
                 ' Vietnam': 4,
                 ' Thailand': 5, ' Ecuador': 6, ' Poland': 7, ' Columbia': 8, ' Yugoslavia': 9, ' Laos': 10,
                 ' Canada': 11,
                 ' Japan': 12, ' Nicaragua': 13, ' France': 14, ' China': 15, ' Cuba': 16, ' Jamaica': 17,
                 ' Hungary': 18,
                 ' Panama': 19, ' United-States': 20, ' Philippines': 21, ' Haiti': 22, ' ?': 0, ' Cambodia': 24,
                 ' Greece': 25,
                 ' Scotland': 26, ' Ireland': 27, ' Peru': 28, ' Hong Kong': 29, ' Taiwan': 30, ' Honduras': 31,
                 ' Germany': 32,
                 ' Italy': 33, ' South Korea': 34, ' India': 35, ' Mexico': 36, ' Iran': 37, ' Puerto-Rico': 38,
